...
class ErrorCollector:
    ...
    def handle_exchange(self,
        ...
        try:
            ...
            # 再提取 endpoint 和 params
            norm_endpoint, params = self._extract_endpoint_and_params(
                endpoint_full, method, req_body, error_msg
            )

            # 用真实参数值恢复 error 中的占位符
            error_msg = self._restore_error_with_params(error_msg, params)
            ...
            with self.lock:
                if merge_key not in self.merged_records:
                    ...
                    if isinstance(old_params, dict) and isinstance(params, dict):
                        for section in ["query", "body", "path"]:
                            if section not in old_params or not isinstance(old_params.get(section), dict):
                                old_params[section] = {}

                            new_section = params.get(section, {})
                            if isinstance(new_section, dict):
                                for k, v in new_section.items():
                                    if k not in old_params[section]:
                                        old_params[section][k] = v
    ...
    def _restore_error_with_params(self, error_msg: str, params: Dict[str, Any]) -> str:
        """
        把 error 中的 {param} 替换成真实参数值
        例如：
        Blog post with id={id} not found.
        -> Blog post with id=4 not found.
        """
        if not error_msg or not isinstance(params, dict):
            return error_msg
        ...
        for section in ["query", "body", "path"]:
            section_dict = params.get(section, {})
